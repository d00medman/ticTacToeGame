...
/***/ function(module, exports, __webpack_require__) {
	...
	var signOutSuccess = function signOutSuccess(response) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isMoveLegal = function isMoveLegal(space) {
	...
	var getTargetCell = function getTargetCell(event) {
	  var targetCell = $(event.target).attr('data-id');
	  return targetCell;
	};

	// mutates the board in makeMove once a move is confirmed legal
	var markDisplay = function markDisplay(event) {
	  var whichCell = $(event.target).attr('data-id');
	  var whichMark = currentPlayer === true ? 'X' : 'O';
	  $('#' + whichCell).text(whichMark);
	  return whichCell;
	};

	// a series of victory checks. They are ugly and verbose, but the logic is clearer to me this way
	var horizontalVictory = function horizontalVictory() {
	...
	var verticalVictory = function verticalVictory() {
	...
	var diagonalVictory = function diagonalVictory() {
	...
	var isGameTied = function isGameTied() {
	...
	var endgame = function endgame() {
	  ...
	  if (isGameWon()) {
	    var winner = currentPlayer === true ? 'O' : 'X'; // note on this: because of the way that move switches turn, it will be the losers turn when a winner is declared, necessitating this statement
	...
	var updateData = function updateData(ind, val) {
	  return {
	    game: {
	      cell: {
	        index: ind,
	...
	var makeMove = function makeMove(event) {
	  var move = getTargetCell(event);
	  move = parseInt(move);
	  var mark = currentPlayer === true ? 'X' : 'O';
	  var alertMark = currentPlayer !== true ? 'X' : 'O';
	  $('.alert').text('It is ' + alertMark + "'s turn.");
	  var legality = isMoveLegal(move);
	  if (!legality) {
	    ...
	    if (moves === 0) {
	      api.create(createData()).then(ui.createSuccess).then(ui.createFailure);
	    } else {
	      // throwing a 400 error.
	      api.update(updateData(move, mark)).then(ui.updateSuccess).then(ui.updateFailure);
	    }
	...
	var onIndex = function onIndex(event) {
	  event.preventDefault();
	  api.index().then(ui.indexSuccess).catch(ui.indexError);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var indexSuccess = function indexSuccess(response) {
	  var games = response.games.length;
	  $('.thisMany').text(games);
	};
